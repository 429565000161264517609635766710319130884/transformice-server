...
from commons.network.crypto.Cryptography import Cryptography
...
class INLogin(Incoming):
    ...
    def dispatch(self, session, buffer_array):
        packet = Cryptography.identification(buffer_array)
        ...
        #    session.disconnect()
        #    return
        if not Cryptography.access(session.authKey, result_key):
            session.disconnect()
            return
